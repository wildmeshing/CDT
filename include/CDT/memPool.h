...
class N_memory_pool {
    std::vector<uint8_t*>
        data;         // Data storage. Each data[i] is an array of N_blocks
   ...
   public:
    N_memory_pool(size_t _size, size_t _block_size)
        : last(_size), size(_size), block_size(_block_size) {
...
class IN_pool {
    std::vector<uint32_t*>
        data;  // Data storage. Each data[i] is an array of extended_N_blocks
    ...
    void addDataBlockArray(uint32_t tot_size, uint32_t extended_block_size,
                           uint32_t** fs) {
...
//    Let V be the value of the element preceeding the one pointed by A (i.e. V
//    = A[-1]) If V==0 use standard free() else if V==N release a block is the
//    corresponding IN_pool
...
class MultiPool {
    ...
    IN_pool& pickPoolFromSize(uint32_t bs) {
        ...
        //// FOR COMPILERS THAT DO NOT SUPPORT C++20 REPLACE THE ABOVE WITH THE
        ///FOLLOWING
   ...
   public:
    ...
    void* alloc(uint32_t num_bytes) {
        ...
        if (num_els > max_block_size) {
            uint32_t* ptr;
            if ((ptr = (uint32_t*)malloc(sizeof(uint32_t) * (num_els + 1))) ==
                NULL)
                return NULL;
            ptr[0] = 0;
            return ptr + 1;
