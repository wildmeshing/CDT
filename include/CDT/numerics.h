...
class interval_number {
    ...
    static inline __m128d sign_low_mask() {
        return _mm_castsi128_pd(_mm_set_epi64x(LLONG_MIN, 0));
    }
    static inline __m128d sign_high_mask() {
        return _mm_castsi128_pd(_mm_set_epi64x(0, LLONG_MIN));
    }
    static inline __m128d sign_fabs_mask() {
        return _mm_castsi128_pd(_mm_set_epi64x(~LLONG_MIN, ~LLONG_MIN));
    }
    static inline __m128d all_high_mask() {
        return _mm_castsi128_pd(_mm_set_epi64x(0, -1LL));
    }

    __m128d getLowSwitched() const {
        return _mm_xor_pd(interval, sign_low_mask());
    }
   ...
   public:
    ...
    double minus_inf() const {
        return _mm_cvtsd_f64(_mm_shuffle_pd(interval, interval, 1));
    }
    ...
    interval_number operator-() const {
        return interval_number(_mm_shuffle_pd(interval, interval, 1));
    }
    interval_number operator+(const double b) const {
        return interval_number(_mm_add_pd(interval, _mm_set_pd(-b, b)));
    }
    interval_number operator-(const double b) const {
        return interval_number(_mm_sub_pd(interval, _mm_set_pd(-b, b)));
    }
    interval_number operator*(const double b) const;
    interval_number operator/(const double b) const;
    interval_number& operator+=(const interval_number& b) {
    ...
    friend inline interval_number min(const interval_number& a,
                                      const interval_number& b);
    friend inline interval_number max(const interval_number& a,
                                      const interval_number& b);
    ...
    int sign() const {
        ...
        __m128i r = _mm_castpd_si128(
            _mm_blendv_pd(_mm_castsi128_pd(_mm_set1_epi32(1)),
                          _mm_castsi128_pd(_mm_set1_epi32(-1)), m));
    ...
    //_mm_castsi128_pd(_mm_set1_epi32(-1)), m)); 	return
    //_mm_cvtsi128_si32(r); }
   ...
   public:
    ...
    interval_number operator+(const double b) const {
        return interval_number(min_low - b, high + b);
    }
    interval_number operator-(const double b) const {
        return interval_number(min_low + b, high - b);
    }
    ...
    friend inline interval_number min(const interval_number& a,
                                      const interval_number& b) {
        return interval_number(std::max(a.min_low, b.min_low),
                               std::min(a.high, b.high));
    }

    friend inline interval_number max(const interval_number& a,
                                      const interval_number& b) {
        return interval_number(std::min(a.min_low, b.min_low),
                               std::max(a.high, b.high));
    }

    bool operator>(const double b) const { return (min_low < -b); }
    bool operator==(const double b) const {
        return (high == b && min_low == -b);
    }
...
#define AllocDoubles(n) \
    ((double*)expansionObject::mempool.alloc((n) * sizeof(double)))
...
class expansionObject {
   public:
    ...
    static void Quick_Two_Sum(const double a, const double b, double& x,
                              double& y) {
        x = a + b;
        y = b - (x - a);
    }
    ...
    static void Two_One_Sum(const double a1, const double a0, const double b,
    ...
    static void Two_Prod_PreSplit(double a, double b, double _bh, double _bl,
                                  double& x, double& y) {
        double _ah, _al;
        x = a * b;
        Split(a, _ah, _al);
    ...
    static void Two_Product_2Presplit(double a, double _ah, double _al,
    ...
    // 'x' is known
    static void Two_Diff_Back(const double a, const double b, double& x,
                              double& y) {
        double _bv;
        _bv = a - x;
        y = (a - (x + _bv)) + (_bv - b);
    ...
    // components.
    static void Two_Two_Prod(const double a1, const double a0, const double b1,
    ...
    // Same as above, but 'h' is allocated internally. The caller must still
    // call 'free' to release the memory.
    static int Gen_Sum_With_Alloc(const int elen, const double* e,
    ...
    // components of result 'h' must be allocated by the caller with at least
    // elen+flen components.
    static int Gen_Diff(const int elen, const double* e, const int flen,
                        const double* f, double* h);

    // Same as above, but 'h' is allocated internally. The caller must still
    // call 'free' to release the memory.
    static int Gen_Diff_With_Alloc(const int elen, const double* e,
    ...
    static int Gen_Scale(const int elen, const double* e, const double b,
                         double* h);

    // [h] = [e] * 2		Multiplies an expansion by 2
    // 'h' must be allocated by the caller with at least elen components. This
    // is exact up to overflows.
    static void Double(const int elen, const double* e, double* h) {
        for (int i = 0; i < elen; i++) h[i] = 2 * e[i];
    }
    ...
    static void ExactScale(const int elen, double* e, const double n) {
        for (int i = 0; i < elen; i++) e[i] *= n;
    }

    // [h] = [a] * [b]
    // 'h' must be allocated by the caller with at least 2*alen*blen components.
    static int Sub_product(const int alen, const double* a, const int blen,
                           const double* b, double* h);

    // [h] = [a] * [b]
    // 'h' must be allocated by the caller with at least MAX(2*alen*blen, 8)
    // components.
    static int Gen_Product(const int alen, const double* a, const int blen,
                           const double* b, double* h);

    // Same as above, but 'h' is allocated internally. The caller must still
    // call 'free' to release the memory.
    static int Gen_Product_With_Alloc(const int alen, const double* a,
    ...
    static int Double_With_PreAlloc(const int elen, const double* e, double** h,
                                    const int hlen);

    static int Gen_Scale_With_PreAlloc(const int elen, const double* e,
                                       const double& b, double** h,
                                       const int hlen);

    static int Gen_Sum_With_PreAlloc(const int elen, const double* e,
                                     const int flen, const double* f,
                                     double** h, const int hlen);

    static int Gen_Diff_With_PreAlloc(const int elen, const double* e,
                                      const int flen, const double* f,
                                      double** h, const int hlen);

    static int Gen_Product_With_PreAlloc(const int alen, const double* a,
                                         const int blen, const double* b,
                                         double** h, const int hlen);
    ...
    static void print(const int elen, const double* e) {
        for (int i = 0; i < elen; i++) printf("%e ", e[i]);
        printf("\n");
    }
};
...
class bignatural {
   ...
   public:
    ...
    // Long division
    bignatural divide_by(const bignatural& divisor,
   ...
   protected:
    ...
    // Count number of zeroes on the right of the last 1 in the least
    // significant limb Assumes that number is not zero and last limb is not
    // zero!
...
class bigfloat {
    bignatural mantissa;  // .back() is less significant. Use 32-bit limbs to
                          // avoid overflows using 64-bits
    int32_t exponent;  // In principle we might still have under/overflows, but
                       // not in practice
   ...
   public:
    ...
    bigrational operator*(const bigrational& r) const {
        if (sign == 0 || r.sign == 0)
            ...
            return bigrational(numerator * r.numerator,
                               denominator * r.denominator, sign * r.sign);
    }
    ...
    bool operator>(const bigrational& r) const {
        return (sign > r.sign ||
                (sign > 0 && r.sign > 0 && hasGreaterModule(r)) ||
                (sign < 0 && r.sign < 0 && r.hasGreaterModule(*this)));
    }

    bool operator>=(const bigrational& r) const {
        return (sign > r.sign ||
                (sign > 0 && r.sign > 0 && hasGrtrOrEqModule(r)) ||
